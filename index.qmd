...
Of the candidates who preferred nephrology `r delta_df |> filter(Variable == "Applicants Prefer Nephrology Matched") |> pull(AY_2023)` obtained nephrology fellowship positions in AY 2023, out of the total `r delta_df |> filter(Variable == "Total Matched") |> pull(AY_2023)` candidates. Allopathic residents entering nephrology `r if_else(delta_df[15, 3] > delta_df[15, 2], "increased", "declined")` `r delta_df |> filter(Variable == "US MD Matched") |> pull(pct_diff) |> abs() |> scales::percent()` from AY 2022, with IMGs `r if_else(delta_df[18, 3] > delta_df[18, 2], "up", "down")` `r delta_df |> filter(Variable == "IMG Matched") |> pull(pct_diff) |> abs() |> scales::percent()`, DOs `r if_else(delta_df[16, 3] > delta_df[16, 2], "up", "down")` `r delta_df |> filter(Variable == "DO Matched") |> pull(pct_diff) |> abs() |> scales::percent()`, and US-IMGs `r if_else(delta_df[17, 3] > delta_df[17, 2], "up", "down")` `r delta_df |> filter(Variable == "US-IMG Matched") |> pull(pct_diff) |> abs() |> scales::percent()` (see [Table 1](#tab1)). The ratio of nephrology candidates `r if_else((delta_df[10, 3] / delta_df[1, 3]) == 0.7, "remained steady at", "changed to")` `r (delta_df[10, 3] / delta_df[1, 3]) |> round(digits = 2)` candidate/position.
...
In AY 2023, `r delta_df |> filter(Variable == "Offered Tracks") |> pull(AY_2023)` were offered in the nephrology Match, of which `r delta_df |> filter(Variable == "Percent Filled Tracks") |> pull(AY_2023) |> scales::percent()` filled all of their available slots. There were `r delta_df |> filter(Variable == "Offered Positions") |> pull(AY_2023)` positions offered in the AY 2023 nephrology Match, `r if_else(delta_df[1, 3] > delta_df[1, 2], "an increase", "a decrease")` of `r delta_df |> filter(Variable == "Offered Positions") |> pull(pct_diff) |> abs() |> scales::percent()` over AY 2022 (see [Table 2](#tab2)).
